...
void solve() {
    ...
    for(int i = 2; i <= n; i++) {
        ...
        // d3(i, prefeven[i], prefodd[i])
    }
    int suffodd[n + 1], suffeven[n + 1];
    suffodd[n] = a[n];
    suffeven[n] = 0;
    for(int i = n - 1; i >= 1; i--) {
        if(i & 1) {
            suffodd[i] = suffodd[i + 1] + a[i];
            suffeven[i] = suffeven[i + 1];
        }
        else {
            suffeven[i] = suffeven[i + 1] + a[i];
    ...
    for(int i = 1; i <= n; i++) {
        // if(i & 1) {
            sum = max({sum, prefodd[i] + suffeven[i], prefeven[i] + suffodd[i]});
        // }
        // else {
            // sum = max({sum, })
        // }
