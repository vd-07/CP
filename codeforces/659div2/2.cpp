...
void solve() {
    ...
    for(int i = 0; i < 2 * k; i++) {
        change[i] = i;
        if(i > k)
            change[i] = 2 * k - change[i];
        if(change[i] + dist[0] <= l)
            safe.insert(i);
        // d1(change[i])
    }
    // for(auto e:safe) {
    //     d1(e)
    // }
    //for any point x, I need to calculate the time at which she can be present there
    set<int> isthere;
    int time;
    for(int i = 1; i < n; i++) {
        // if(i == 5) {
        ...
        for(auto e:safe) {
            ...
            //         d1(e)
            //     }
            for(int ti = 1; ti <= 2 * k; ti++) {
                time = e + ti;
                time %= 2 * k;
                
                if(isthere.find(time) != isthere.end())
                    break;
                if(dist[i] + change[time] <= l)
                    isthere.insert(time);
                else break;
            }
