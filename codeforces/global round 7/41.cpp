...
void solve(){
    ...
    string p1=s.substr(0,count)+s.substr(n-count-1,count+1);
    count=1;
    for(int i=0;i<n;i++){
        if(s[i]==s[0]){
            ll ans=1;
            while(s[i-ans]==s[ans]&&ans<=(i+1)/2){
                ans++;
            }
            if(ans-1==(i+1)/2)
            count=max(2*(ans-1)+(i&1),count);
        }
    }
    string p2=s.substr(0,count);
    count=1;
    for(int i=n-1;i>=0;i--){
        if(s[i]==s[n-1]){
            ll ans=1;
            while(s[i+ans]==s[n-ans-1]&&ans<=(n-i)/2){
                ans++;
            }
            if(ans-1==(n-i)/2)
            count=max(2*(ans-1)+!2*(i&1),count);
        }
    }
    string p3=s.substr(n-count-1,count);
    cout<<p1<<" "<<p2<<" "<<p3<<"\n";
