...
void solve(){
   ...
   ll ans = 0;
   for(int i = 1; i <= n; i++) {
    ll combo = (factorial[n] * modInverse(factorial[i], mod) % mod * modInverse(factorial[n - i], mod) % mod) % mod;
    ll sa = factorial[(n - i) * k + i], 
    re = factorial[k];
    re = power(re, n - i, mod) % mod;
    if(i & 1) {
      ans = (ans + combo % mod * sa % mod * modInverse(re, mod) % mod) % mod;
    }
    else {
      ans = (ans - combo % mod * sa % mod * modInverse(re, mod) % mod) % mod; 
    }
