...
void solve(){
  ...
  // make_heap(loc.begin(), loc.end());
  // cout << (0 % 5);
  // sort(b, b + n);
  // for(int i = 0; i < n; i++) {
  //   auto j = lower_bound(b, b + n, a[i]) - b;
  //   auto p = upper_bound(b, b + n, a[i]) - b;
  //   if(p == n) p--;
  //   // d3(a[i], j , p);
  //   if(j / k == p / k && j % k && p % k) {
  //     cout << "no\n";
  ...
  // for(int i = 0; i < n; i++) {
    ...
    // loc2[a[i]].pb(i);
  // }
  // for(auto itr1 = loc1.begin(), itr2 = loc2.begin(); itr1 != loc1.end(); itr1++, itr2++) {
  //   vector<int> v1 = itr1->second, v2 = itr2->second;
  //   for(int i = 0; i < v1.size(); i++) {
  //     d3(itr1->first, v1[i], v2[i])
  //     if((v1[i] - v2[i]) % k) {
  //       cout << "no\n";
  //       return ;
  //     }
